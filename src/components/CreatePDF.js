...
import unidecode from 'unidecode' // chwilowe obejscie problemu z niewspieranym enkodowaniem utf-8 i polskimi znakami
...
function createHeaders(keys) {
  ...
  for (var i = 0; i < keys.length; i += 1) {
    result.push({
      ...
      name: keys[i],
      prompt: keys[i],
      width: i == 0 ? 16 : i == 1 ? 69 : i == 2 || i == 5 ? 26 : 32, // dostosowanie szerokosci kolumn w zaleznosci od jej zastosowania
      align: "center",
      padding: 0,
    });
  }
...
var generateData = function(amount) {
  ...
  for (var count = 0; count < amount; count += 1) {
    // ponizej uzywam formatu html `${}`, poniewaz bez tego niepoprawnie odczytywane sa zmienne Number
    data = {
...
export function fetchInvoiceData(providedData) {
  invoiceData = JSON.parse(providedData)
  // console.log(invoiceData)
  tableData = invoiceData?.invoiceData?.Wykonane_uslugi_czesci
